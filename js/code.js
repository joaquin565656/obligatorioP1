...
function login() {
  ...
  document.querySelector("#contenedorPanelSuperior").innerHTML =
    contenedorPanelSuperior;
}
...
function obtenerListaConciertosFiltro() {
  ...
  if (buscador.length > 0) {
    for (let concierto of listaConciertos) {
      if (
        concierto.nombre.toUpperCase().indexOf(buscador.toUpperCase()) != -1
      ) {
        ...
        if (!existe) conciertos.push(concierto);
      }
    }
  } else {
...
function cargarConciertosDisponibles() {
  ...
  if (conciertos.length <= 0)
    document.querySelector(
      "#contenedorConcierto"
    ).innerHTML = `<div> No hay conciertos que coincidan con la búsqueda`;
  ...
  for (let concierto of conciertos) {
    ...
    ${concierto.oferta ? '<span class="badge-oferta"> En oferta</span>' : ""}
      ...
      <div class="datosConcierto">
        ...
        <button class="btnReservar" onClick="abrirPopUpReserva('${
          concierto.id
...
function abrirPopUpReserva(idConcierto) {
  ...
  document.querySelector("#PopUpReservaNombreConcierto").innerText =
    concierto.nombre;
  document.querySelector(
    "#PopUpReservaArtista"
  ).innerHTML = `<b>Artista:</b> ${concierto.artista}`;
  document.querySelector(
    "#PopUpReservaPrecio"
  ).innerHTML = `<b>Precio:</b> $${concierto.precio}`;
  document.querySelector(
    "#PopUpReservaCupos"
  ).innerHTML = `<b>Cupos disponibles:</b> ${concierto.cuposDisponibles}`;
...
function confirmarReserva() {
  ...
  sistema.crearReserva(
    sistema.obtenerUsuarioLogueado(),
    conciertoSeleccionado,
    cantidad,
    cantidad * conciertoSeleccionado.precio
  );
  iniciarCliente();
